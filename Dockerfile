...
RUN set -eux; \
  if [ "${BASE_IMAGE_TAG%%.*}" = "8" ]; then \
    COMPOSER_MEMORY_LIMIT=-1 composer require "twig/extensions:^1.5.4" "symfony/mime:^4.3"; \
  fi; \
  if [ "${BASE_IMAGE_TAG%%.*}" = "9" ]; then \
    COMPOSER_MEMORY_LIMIT=-1 composer require "twig/extensions:^1.5.4" "symfony/mime:^5.1.0"; \
  fi;
